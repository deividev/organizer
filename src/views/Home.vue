<template>
  <div>
    ...
    <div>
      <h1>{{longitud}}</h1>
      <button @click="startTrackPosition()" class="btn">Start</button>
...
export default {
  ...
  methods: {
    ...
    centerMap(position) {
      ...
      this.map.getView().setCenter(fromLonLat(positionUser));
      if (!this.circle) {
        debugger
        this.currentPosition = fromLonLat(positionUser);
        this.createPoint(positionUser);
        return;
      }
      this.circle.setCenter(fromLonLat(positionUser));
